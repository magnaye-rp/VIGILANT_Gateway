...
DB_PATH = os.getenv("VIGILANT_DB_PATH", "/home/vigilant_admin/vigilant/logs/vigilant.db")

# Only these keys are runtime-configurable from UI clients.
CONFIG_DEFAULTS = {
    "block_harmful": True,
...
ALLOWED_CONFIG_KEYS = set(CONFIG_DEFAULTS.keys())


def _format_uptime() -> str:
    """Return uptime in the required `Xh Ym` format."""
...
def _service_statuses() -> dict:
    """Detect proxy/dashboard processes from active process list."""
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            cmdline_items = [str(item).lower() for item in (info.get("cmdline") or [])]
            joined_cmdline = " ".join(cmdline_items)

            if not proxy_active and ("mitmdump" in name or "mitmdump" in joined_cmdline):
                proxy_active = True

            # Ignore this process; only report peer dashboard workers for reliability.
            if (
                not dashboard_active
                and info.get("pid") != current_pid
                and ("app.py" in joined_cmdline)
            ):
...
def _read_runtime_config() -> dict:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        if not _table_exists(conn, "config_settings"):
            return config
    ...
    for key, raw_value in rows:
        ...
        try:
            ...
            # Keep secure defaults if persisted data is malformed.
...
def _stats_from_db() -> dict:
    """Collect 24h active clients + category distribution from traffic_log."""
    categories = ["Educational", "Productive", "Distracting", "Harmful"]
    stats = {
        "connected_devices": 0,
        "traffic_distribution": {
            category: {"count": 0, "percentage": 0.0} for category in categories
        },
        "total_requests": 0,
    }
    ...
    with sqlite3.connect(DB_PATH) as conn:
        if not _table_exists(conn, "traffic_log"):
            return stats
        ...
        total_row = conn.execute("SELECT COUNT(*) FROM traffic_log").fetchone()
        total_requests = int(total_row[0] or 0)
        stats["total_requests"] = total_requests

        count_rows = conn.execute(
            """
            SELECT LOWER(TRIM(category)) AS normalized_category, COUNT(*) AS row_count
            FROM traffic_log
            GROUP BY LOWER(TRIM(category))
            """
        ).fetchall()

        counts_by_key = {str(row[0] or ""): int(row[1] or 0) for row in count_rows}
...
@app.route("/")
def index():
    """Main web admin dashboard shell."""
    return render_template("pages/dashboard.html", server_ip=SERVER_IP)


@app.route("/api/stats")
def api_stats():
    """Unified lightweight payload for mobile + web dashboard clients."""
    try:
        db_stats = _stats_from_db()
        payload = {
            "server_ip": SERVER_IP,
            "server_status": _service_statuses(),
            "connected_devices": db_stats["connected_devices"],
            "online_duration": _format_uptime(),
            "traffic_distribution": db_stats["traffic_distribution"],
            "total_requests": db_stats["total_requests"],
            "analysis_context": {
                "method": "passive_dns_tls_sni",
...
def api_config():
    """Runtime configuration API for mobile/basic and web advanced views."""
    if request.method == "GET":
        return jsonify(_read_runtime_config())
    ...
    for key, raw_value in payload.items():
        if key not in ALLOWED_CONFIG_KEYS:
            # Explicitly filter out unsupported and infrastructure-oriented keys.
